...
| 测试场景 | 测试数据 | 预期结果 |
| :--- | :--- | :--- |
| 示例1-多次匹配 | 文本：DogCatDogFishDogBird<br>模式：Dog | 次数：3，索引：[0,6,13] |
| 示例2-多次匹配 | 文本：DataStructDataAlgoStruct<br>模式：Struct | 次数：2，索引：[4,18] |
| 空值和null处理 | 文本或模式为null/空串 | 次数：0，索引：空列表 |
| 模式串过长 | 文本：short，模式：longerPattern | 次数：0，索引：空列表 |
| 无匹配场景 | 文本：abcdefg，模式：xyz | 次数：0，索引：空列表 |
| 首尾边界匹配 | 文本：prefixContentSuffix<br>模式：prefix/Suffix | 匹配首尾位置正确 |
| 重叠匹配 | 文本：AAAAA，模式：AA | 次数：4，索引：[0,1,2,3] |
| 特殊字符匹配 | 文本：Hello，World!@2025#Java<br>模式：@2025 | 次数：1，索引：[14] |
| 单字符匹配 | 文本：A，模式：A/B | A匹配，B不匹配 |

**表 4.2 文件操作测试用例**

| 测试场景 | 测试数据 | 预期结果 |
| :--- | :--- | :--- |
| 正常保存文本 | 多行文本内容 | 文件创建成功,内容一致 |
| 保存空内容 | 空字符串 | 创建空文件 |
| 保存null内容 | null值 | 保存为空字符串 |
| null文件对象 | 文件对象为null | 抛出IllegalArgumentException |
| 中文内容保存 | 中文文本 | UTF-8编码正确保存 |
| 覆盖已有文件 | 已存在的文件 | 成功覆盖旧内容 |
| 正常读取文本 | 多行文本文件 | 读取内容与原内容一致 |
| 读取空文件 | 空文件 | 返回空字符串 |
| 中文内容读取 | 含中文的文件 | UTF-8解码正确 |
| 读取null文件 | 文件对象为null | 抛出IllegalArgumentException |
| 文件不存在 | 不存在的文件路径 | 抛出FileNotFoundException |
| 读取目录路径 | 目录路径 | 抛出IllegalArgumentException |
| 读写集成测试 | 保存后立即读取 | 内容完全一致 |
| 多行文本处理 | 5行文本 | 换行符保持正确 |
| 特殊字符处理 | 符号和制表符 | 特殊字符保持正确 |

**表 4.3 系统集成与性能测试用例**

| 测试场景 | 测试数据 | 预期结果 |
| :--- | :--- | :--- |
| 完整流程测试1 | 文本：DogCatDogFishDogBird<br>模式：Dog | 创建、统计、查询功能正常 |
| 完整流程测试2 | 文本：DataStructDataAlgoStruct<br>模式：Struct | 创建、统计、查询功能正常 |
| 大规模性能测试 | 文本长度：100,000字符<br>模式：ABCDE | 查询时间<1秒 |
